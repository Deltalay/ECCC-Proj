from typing import AnyStr, BinaryIO, Dict, List, NamedTuple, Optional, Union, IO, Any
import os
import numpy as np
import pandas as pd
import pydicom as dicom
from skimage.exposure import rescale_intensity
FilePathOrBuffer = Union[str, os.PathLike, IO[Any]]
...
def evaluate(
    ...
    df_labels = df_labels.reset_index().set_index(
        ["StudyUID", "View"]).sort_index()
    df_boxes = df_boxes.reset_index().set_index(
        ["StudyUID", "View"]).sort_index()
    df_pred = df_pred.reset_index().set_index(
        ["StudyUID", "View"]).sort_index()
    ...
    for box_pred in df_pred.itertuples():
        ...
        view_slice_offset = df_boxes.loc[[
            box_pred.Index], "VolumeSlices"].iloc[0] / 4
        tp_boxes = [
            b
        ...
        if len(tp_boxes) > 0:
            ...
            df_pred.loc[df_pred["index"] ==
                        box_pred.index, ("TP", "GTID")] = (1, tp_i)
